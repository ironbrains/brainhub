defmodule BrainhubWeb.TeamMembership do
  use Brainhub.Web, :model

  schema "team_memberships" do
    field :initiator_id, :integer, virtual: true

    belongs_to :user, BrainhubWeb.User
    belongs_to :team, BrainhubWeb.Team
  ...
  defp validate_users(changeset) do
    case changeset do
      %Ecto.Changeset{valid?: true, changes: %{initiator_id: initiator_id, user_id: user_id, team_id: team_id}} ->
        team = Brainhub.Repo.get(BrainhubWeb.Team, team_id) |> Brainhub.Repo.preload(:company)
  ...
  defp validate_initiator(changeset, company_id, initiator_id) do
    if BrainhubWeb.Company.management?(company_id, initiator_id) do
      changeset
    else
  ...
  defp validate_member(changeset, company_id, user_id) do
    if BrainhubWeb.Company.employer?(company_id, user_id) do
      changeset
    else
