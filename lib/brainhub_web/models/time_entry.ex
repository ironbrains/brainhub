defmodule BrainhubWeb.TimeEntry do
  ...
  def created_by(%BrainhubWeb.User{id: user_id}), do: created_by user_id
  ...
  def duration_for(%BrainhubWeb.User{id: user_id}, date), do: duration_for user_id, date

  def duration_for(user_id, date) do
    BrainhubWeb.TimeEntry
    |> where([t], t.user_id == ^user_id)
    |> BrainhubWeb.TimeEntry.duration_for_date(date)
  end
end
